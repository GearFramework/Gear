...
Сайт: <a href="http://www.gearframework.ru" target="_blank">www.gearframework.ru</a>
Контакт: <a href="mailto:dev@gearframework.ru" target="_blank">dev@gearframework.ru</a>
...
либо в консоли
```
/usr/bin/php /var/www/index.php -e processName
```
...
<b>preloads</b> служит для подключения критически важных библиотек классов, модулей и компонентов, экземляры которых будут созданы на этапе конфигурирования, например, по-умолчанию здесь определены: базовые классы, а также автозагрузчик, обработчики исключений и ошибок. Данный раздел также может содержать три подраздела: `library`, `modules`, `components`.

<b>modules</b> служит для определения модулей.

<b>components</b> служит для определения компонентов.

Кроме того, каждый модуль или компонент, да и любой объект наследованный от класса '\gear\library\GObject' может конфигурироваться. Конфигурация таких объектов является статической и доспупна через метод `GObject::i()`, который принимает один аргумент - название конфигурационного параметра. Конфигурационные, т.е. статические параметры, определяются в разделе `class`, обычные свойства объекта - в основном разделе:
...
echo \gear\Core::c('myComponent')->property1; \\ Выведет на экран 'value1'
```
```php
<?php
...
echo \gear\Core::c('myComponent')->i('param'); \\ Выведет на экран 'paramValue'
``` 

Кроме того, конфигурироваться элементы могут непосредственно при реализации класса
...
class GMyComponent extends GComponent
{
...
Самостоятельная сущность, у которой могут быть свои компоненты, бибилиотеки, шаблоны отображения и т.д. Добавление модулей происходит в конфигурационном файле в разеделе `modules`.
