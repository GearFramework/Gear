...
\gear\Core::app()->run();
```
Первой строкой подключается ядро фреймворка. За ней следует инициализация ядра посредством вызова метода `init()`. Процесс инициализации представляет собой чтение указанного конфигурационного файла, подключение необходимых библиотек, модулей, компонентов.
