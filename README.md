...
Все запросы должны проходить через единственный файл index.php, который в общем случае, располагается в `DOCUMENT_ROOT` вашего Apache-сервера (допустим это будет `/var/www`). Помимо index.php рядом могут располагаться конфигурационные файлы `config.production.php` и/или `config.debug.php`, в зависимости от режима работы приложения (config.production.php - настройки приложения для работы в продакшене, config.debug.php - настройки приолжения для работы в режиме разработки и отладки).
...
\gear\Core::app()->run();
```
Первой строкой подключается ядро фреймворка. За ней следует инициализация ядра посредством вызова метода `init()`. Процесс инициализации представляет собой чтение указанного конфигурационного файла, исходя из его настроек - подключение необходимых библиотек, модулей, компонентов.
