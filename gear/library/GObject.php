...
class GObject
{
    ...
    public function __set($name, $value) {
        ...
        if (method_exists($this, $setter))
            $this->$setter($value);
        else if (method_exists($this, 'attachEvent') && preg_match('/^on[A-Z]/', $name))
            $this->attachEvent($name, $value);
        else if (method_exists($this, 'registerComponent') && $value instanceof IComponent)
                $this->registerComponent($name, $value);
        else if (method_exists($this, 'attachBehavior') && (is_callable($value) || $value instanceof \gear\interfaces\IBehavior))
                $this->attachBehavior($name, $value);
        else if (method_exists($this, 'installPlugin') && $value instanceof \gear\interfaces\IPlugin)
                $this->installPlugin($name, $value);
        else if ($this instanceof IBehavior || $this instanceof IPlugin)
    ...
    public function __get($name) {
        ...
        if (method_exists($this, $getter))
            $value = $this->$getter();
        else if (method_exists($this, 'trigger') && preg_match('/^on[A-Z]/', $name))
                $value = $this->trigger($name);
        else if (method_exists($this, 'isComponentRegistered') && $this->isComponentRegistered($name))
                $value = $this->c($name);
        else if (method_exists($this, 'b') && $this->isBehavior($name))
                $value = $this->b($name);
        else if (method_exists($this, 'p') && $this->isPluginRegistered($name))
                $value = $this->p($name);
        else {
            if (array_key_exists($name, $this->_properties))
                $value = $this->_properties[$name];
            else if (is_object($this->_owner))
    ...
    public function __call($name, $args) {
        ...
        } else if (method_exists($this, 'c') && $this->isComponentRegistered($name)) {
    ...
    public function __unset($name) {
        ...
        else if (method_exists($this, 'isPluginRegistered') && $this->isPluginRegistered($name))
...
trait TComponents
{
    ...
    protected function _getKey($name) {
        return get_class($this) . '.components.' . $name;
    }
}
...
trait TBehaviors
{
    ...
    public function attachBehavior($name, $behavior) {
        ...
        else if ($behavior instanceof \Closure)
                // $behavior is object instance of \Closure
                $this->_behaviors[$name] = $behavior->bindTo($this, $this);
        else if ($behavior instanceof \gear\interfaces\IBehavior || is_callable($behavior))
