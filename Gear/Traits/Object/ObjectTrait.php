...
namespace Gear\Traits\Object;

use Gear\Core;
use Gear\Interfaces\ComponentContainedInterface;
use Gear\Interfaces\ComponentInterface;
use Gear\Interfaces\ContainComponentsInterface;
use Gear\Interfaces\ContainPluginsInterface;
...
trait ObjectTrait
{
     ...
     * GObject constructor.
     *
     * @param iterable $properties свойства создаваемого объекта
     * @param null|ObjectInterface $owner владелец объекта
     * @since 0.0.1
     * @version 2.0.0
     */
    protected function __construct(iterable $properties = [], ?ObjectInterface $owner = null)
    {
    ...
    public function __get(string $name)
    {
        ...
        if (method_exists($this, $getter)) {
            return $this->$getter();
        } elseif (preg_match('#^on[A-Z]#', $name) && method_exists($this, 'trigger')) {
            return $this->trigger($name, new Event($this, ['target' => $this]));
        } elseif ($this instanceof ContainComponentsInterface && $this->isComponent($name)) {
            return $this->c($name);
        } elseif ($this instanceof ContainPluginsInterface  && $this->isPlugin($name)) {
            return $this->p($name);
        } else {
